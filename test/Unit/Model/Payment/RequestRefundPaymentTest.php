<?php

namespace mvd\Vipps\Tests\Unit\Model\Payment;

use mvd\Vipps\Model\Payment\CustomerInfo;
use mvd\Vipps\Model\Payment\MerchantInfo;
use mvd\Vipps\Model\Payment\RequestRefundPayment;
use mvd\Vipps\Model\Payment\Transaction;
use mvd\Vipps\Tests\Unit\Model\ModelTestBase;

class RequestRefundPaymentTest extends ModelTestBase
{

    /**
     * @var \mvd\Vipps\Model\Payment\RequestRefundPayment
     */
    protected $model;

    /**
     * {@inheritdoc}
     */
    public function setUp()
    {
        parent::setUp();
        $this->model = new RequestRefundPayment();
    }

    /**
     * @covers \mvd\Vipps\Model\Payment\RequestRefundPayment::setMerchantInfo()
     * @covers \mvd\Vipps\Model\Payment\RequestRefundPayment::getMerchantInfo()
     */
    public function testMerchantInfo()
    {
        $this->assertNull($this->model->getMerchantInfo());
        $this->assertInstanceOf(RequestRefundPayment::class, $this->model->setMerchantInfo(new MerchantInfo()));
        $this->assertInstanceOf(MerchantInfo::class, $this->model->getMerchantInfo());
    }

    /**
     * @covers \mvd\Vipps\Model\Payment\RequestRefundPayment::setTransaction()
     * @covers \mvd\Vipps\Model\Payment\RequestRefundPayment::getTransaction()
     */
    public function testTransaction()
    {
        $this->assertNull($this->model->getTransaction());
        $this->assertInstanceOf(RequestRefundPayment::class, $this->model->setTransaction(new Transaction()));
        $this->assertInstanceOf(Transaction::class, $this->model->getTransaction());
    }
}
